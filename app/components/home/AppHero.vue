...
const toggleFavorite = async () => {
  ...
  if (!session.value) {
    toast.add({ title: 'Connexion requise', description: 'Vous devez être connecté pour ajouter aux favoris', color: 'red' })
    return
  }
  favoriteLoading.value = true
  try {
    const response: any = await $fetch('/api/favorites', { method: 'POST', body: { eventId: currentEvent.value.id } })
    isFavorite.value = response.status === 'added'
    toast.add({ title: isFavorite.value ? 'Ajouté aux favoris ❤️' : 'Retiré des favoris', color: isFavorite.value ? 'green' : 'orange' })
  } catch {
    toast.add({ title: 'WeLoveEvents', description: 'Vous devez être connecté pour ajouter aux favoris', color: 'red' })
    ...
    <div class="carousel-backgrounds">
      ...
      >
        <NuxtImg
          ...
          :src="event.image.url" :alt="event.title"
          class="bg-image" loading="eager" quality="85"
        />
    ...
    <div class="hero-content" :class="{ 'hero-content--ready': isReady }">
      <div class="container">
        ...
        <div class="event-showcase">
          ...
          <p class="event-subtitle">{{ currentEvent?.description }}</p>
          ...
          <div class="event-actions">
            ...
            <button @click="toggleFavorite" :disabled="favoriteLoading" class="btn-secondary"
              :class="isFavorite ? 'bg-red-100 text-red-600' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'">
              <svg class="w-5 h-5" :class="isFavorite ? 'fill-red-600' : 'stroke-current'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/>
              </svg>
              <span class="text-sm font-medium">{{ isFavorite ? 'En favori' : 'Favoris' }}</span>
            </button>
          </div>
        </div>
        ...
        <div class="carousel-controls">
          ...
          <div class="carousel-dots">
            <button
              v-for="(event, index) in featuredEvents" :key="event.id"
              class="carousel-dot" :class="{ 'carousel-dot--active': currentSlide === index }"
              @click="goToSlide(index)" :aria-label="`Aller à l'événement ${Number(index) + 1}`"
            />
          </div>
        ...
        <div class="categories-section hidden sm:block">
          ...
          <div class="categories-grid">
            <button v-for="category in categories" :key="category.id" class="category-card" @click="goToCategory(category.id)">
...
/* ── Toute la section utilise Outfit ── */
.hero-carousel,
.hero-carousel * {
...
.bg-overlay {
  ...
  background: linear-gradient(135deg, rgba(0,0,0,0.5) 0%, rgba(0,0,0,0.3) 40%, rgba(0,0,0,0.6) 100%);
}
...
.hero-content--ready { opacity: 1; transform: scale(1); }

.container { max-width: 1400px; margin: 0 auto; width: 100%; }

.event-showcase { max-width: 700px; margin-bottom: 3rem; }
...
.event-subtitle {
  ...
  font-weight: 300; color: rgba(255,255,255,0.9); margin-bottom: 2rem;
}

.event-meta { display: flex; flex-wrap: wrap; gap: 2rem; margin-bottom: 2.5rem; }

.meta-item { display: flex; align-items: center; gap: 0.75rem; color: white; font-size: 1rem; }

.meta-icon { width: 1.25rem; height: 1.25rem; }

.event-actions { display: flex; flex-wrap: wrap; gap: 1rem; }
...
.btn-primary:hover { transform: translateY(-2px); box-shadow: 0 15px 50px -10px rgba(255,255,255,0.7); }
...
.carousel-dot {
  ...
  border-radius: 50%; cursor: pointer; transition: all 0.3s; padding: 0;
}
.carousel-dot--active { width: 2rem; background: white; border-radius: 9999px; }

.categories-section {
  background: rgba(255,255,255,0.05); backdrop-filter: blur(20px);
  border: 1px solid rgba(255,255,255,0.1); border-radius: 1.5rem; padding: 2rem;
}
...
.category-info { flex: 1; display: flex; flex-direction: column; gap: 0.25rem; }

.category-name { font-size: 1rem; font-weight: 600; color: white; }

.category-arrow {
  width: 1.25rem; height: 1.25rem;
  color: rgba(255,255,255,0.5); transition: transform 0.3s;
}
.category-card:hover .category-arrow { color: white; transform: translateX(4px); }
